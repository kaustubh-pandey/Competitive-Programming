...
int main(){
    ...
    while(t--){
        ...
        cin>>n>>k;
        list<int> activeSet;
        unordered_map<int,int,custom_hash> visited;
        int d;
        fo(i,n){
            cin>>d;
            if(!visited[d]){
                visited[d]=1;
                if((int)activeSet.size()==k){
                    activeSet.push_front(d);
                    int toRemove = activeSet.back();
                    activeSet.pop_back();
        ...
        cout<<activeSet.size()<<endl;
        for(auto x:activeSet){
            cout<<x<<" ";
        }
        cout<<endl;
    }
